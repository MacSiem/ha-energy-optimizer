...
(async () => {
  ...
  for (const f of files) {
    ...
    const residual = ['__haToolsSplitDonateInjector', 'SPLIT_TAGS', 'deepFindAll(', 'data-source="ha-tools-split-injector"', forbiddenGlobalEscape, 'window.HAToolsBentoCSS', '_injectDiscovery', 'HAToolsDiscovery', 'ha-tools-discovery.js', 'cdn.jsdelivr.net', "createElement('script", 'createElement("script'].find(token => code.includes(token));
  ...
  for (const t of targets) {
    ...
    try {
      ...
      const localDonateTags = new Set([...localIntroTags, 'ha-purge-cache', 'ha-energy-optimizer', 'ha-energy-insights', 'ha-energy-email']);
